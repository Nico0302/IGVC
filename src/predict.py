...
def prepare_plot(origImage, origMask, predMask, filename):
    ...
    figure.tight_layout()
    figure.show()
    # check to see if the output directory exists, and if not,
    # create it
    if not os.path.exists(os.path.join(config.BASE_OUTPUT, "predictions")):
        os.makedirs(os.path.join(config.BASE_OUTPUT, "predictions"))
    # save the figure to disk
    plt.savefig(os.path.join(config.BASE_OUTPUT, "predictions", filename))
...
def make_predictions(model, imagePath):
    ...
    with torch.no_grad():
        ...
        # mask
        filename = "Image" + imagePath.split(os.path.sep)[-1]
